...
function improveDescription(name, currentDescription) {
  ...
  // Define specific improvements for common strategies
  const improvements = {
...
• Reward yourself when you meet it`
  ...
  if (improved.length > 300) {
    // Try to extract the main action/concept and make bullet points
    const sentences = improved.split('.').filter(s => s.trim().length > 10);
    if (sentences.length > 1) {
      const mainAction = sentences[0].trim() + '.';
  ...
  return new Promise((resolve, reject) => {
    fs.createReadStream(inputPath)
      ...
      .on('data', (row) => {
        ...
        const improvedDescription = improveDescription(row.Name, originalDescription);
        
        if (improvedDescription !== originalDescription) {
          console.log(`✓ Improved: ${row.Name}`);
          updatedCount++;
        }
      ...
      .on('end', () => {
        console.log(`\nProcessed ${strategies.length} strategies`);
        ...
        if (strategies.length > 0) {
          ...
          csvWriter.writeRecords(strategies)
            .then(() => {
              console.log(`\n✓ Saved improved CSV to: ${path.basename(outputPath)}`);
              resolve();
            })
            .catch(reject);
        } else {
